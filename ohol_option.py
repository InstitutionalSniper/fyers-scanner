...
def get_fyers_instance():
    with st.sidebar:
        ...
        if 'access_token' not in st.session_state:
            ...
            st.write("")
            full_url = st.text_input("STEP 2: PASTE REDIRECT URL")
                ...
                if "auth_code=" in full_url:
                    try:
                        ...
                        if 'access_token' in response:
                            ...
                            st.rerun()
                    except: st.error("Invalid URL or Code.")
            return None
...
def get_opt_sym(stock, ltp, type="CE"):
    expiry = "16APR26" # Current Live Expiry
...
st.title("⚡ TERMINAL ALPHA: LIVE OHL SCANNER")
...
if fyers:
    ...
    t1.metric("SESSION", "LIVE", "APRIL 13")
    t2.metric("SCANNER", "15-MIN OHLC")
    t3.metric("EXPIRY", "16-APR-2026")
    t4.metric("STATUS", "CONNECTED ✅")
    ...
    with c_left:
        ...
        if results:
            ...
            # FIXED: 'map' instead of 'applymap'
            st.dataframe(df_final.style.map(color_rows, subset=['ACTION']), height=500, use_container_width=True)
            if st.button("⚡ FORCE REFRESH"): st.rerun()
        else:
            st.warning("No Data Found. Check if market is open.")
