...
def get_fyers_instance():
    if 'access_token' not in st.session_state:
        with st.sidebar:
            ...
            st.markdown(f'''<a href="{auth_url}" target="_blank"><button style="width:100%; height:50px; background: #ED1C24; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:bold;">1. LOGIN FYERS</button></a>''', unsafe_allow_html=True)
            st.write("---")
            full_url = st.text_area("2. PASTE FULL GOOGLE URL HERE", height=100)
...
if fyers:
    # Adani Power, Tata Motors aur baaki bhagne wale stocks
    master_list = ["NSE:NIFTY50-INDEX", "NSE:NIFTYBANK-INDEX", "NSE:ADANIPOWER-EQ", "NSE:TATAMOTORS-EQ", "NSE:RELIANCE-EQ", "NSE:SBIN-EQ", "NSE:HDFCBANK-EQ", "NSE:TATASTEEL-EQ", "NSE:ADANIENT-EQ", "NSE:JINDALSTEL-EQ"]
    ...
    if results:
        ...
        st.markdown(f'<marquee style="color: #00ff00; font-size: 22px; background: #111; padding: 12px; font-family: monospace; font-weight: bold;">🔥 LIVE MOVERS: {ticker_text}</marquee>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            # FIXED: 'applymap' badal kar 'map' kar diya hai
    ...
    st.info("👈 Sidebar mein LOGIN karke FULL GOOGLE URL paste karein.")
